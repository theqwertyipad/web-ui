...
async def run_agent_task(
    webui_manager: WebuiManager, components_dict: Dict[gr.components.Component, Any]
    ...
    task = components_dict.get(user_input_comp, "").strip()
    if not task:
    ...
    mcp_server_config_str = (
        components_dict.get(mcp_server_config_comp) if mcp_server_config_comp else None
    )
    mcp_server_config = (
    ...
    try:
        ...
        if not webui_manager.bu_browser:
            ...
            load_recorder_extension = _get_config_value(
                webui_manager, components_dict, "load_recorder_extension", False
            )
            ...
            # Add workflow recorder extension if requested
            if load_recorder_extension:
                from src.utils import extension_loader

                extension_path = extension_loader.get_extension_path()
                if extension_path:
                    # Create temporary copy of extension to avoid path issues
                    ext_path = extension_loader.create_temp_extension_dir()
                    if ext_path:
                        logger.info(
                            f"Loading workflow recorder extension from: {ext_path}"
                        )
                        extension_args = extension_loader.prepare_browser_args(
                            headless, ext_path
                        )
                        ...
                        logger.warning("Failed to create temp extension directory")
                else:
                    logger.warning("Workflow recorder not found")
...
async def handle_submit(
    ...
    user_input_comp = webui_manager.get_component_by_id("browser_use_agent.user_input")
    user_input_value = components_dict.get(user_input_comp, "").strip()
